...
void OidcTokenVerifier::init()
{
	...
	loadConfig();
	LOG_INF << fname << "configured Dex issuer <" << m_config.issuer
...
void OidcTokenVerifier::loadConfig()
{
	...
	m_config.dexAccessUrl = normalizeIssuer(GET_JSON_STR_VALUE(oidc, "dex_access_url"));
	if (oidc.contains("dex_tls_verify"))
	{
		if (!oidc.at("dex_tls_verify").is_boolean())
			throw std::invalid_argument("OIDC dex_tls_verify must be true or false");
		m_config.dexTlsVerify = oidc.at("dex_tls_verify").get<bool>();
	}
	m_config.dexCaPath = Utility::stdStringTrim(GET_JSON_STR_VALUE(oidc, "dex_ca_path"));
	if (const char *issuer = std::getenv("APPMESH_DEX_ISSUER"); issuer != nullptr && issuer[0] != '\0')
		m_config.issuer = normalizeIssuer(issuer);
	if (const char *accessUrl = std::getenv("APPMESH_DEX_ACCESS_URL"); accessUrl != nullptr && accessUrl[0] != '\0')
		m_config.dexAccessUrl = normalizeIssuer(accessUrl);
	if (const char *caPath = std::getenv("APPMESH_DEX_CA_PATH"); caPath != nullptr && caPath[0] != '\0')
		m_config.dexCaPath = caPath;
	if (const char *tlsVerify = std::getenv("APPMESH_DEX_TLS_VERIFY"); tlsVerify != nullptr && tlsVerify[0] != '\0')
	{
		...
		if (value == "1" || value == "true")
			...
			throw std::invalid_argument("APPMESH_DEX_TLS_VERIFY must be true or false");
	}
	...
	if (m_config.issuer.empty())
		throw std::invalid_argument("Dex issuer is not configured");
	...
	for (const auto &endpoint : {m_config.issuer, m_config.dexAccessUrl, m_config.resourceUrl})
	{
		...
		{
			throw std::invalid_argument(
				"OIDC issuer, dex_access_url, and resource_url must be absolute HTTP(S) URLs without credentials, query, or fragment");
		}
	}
	...
	if (!m_dexSslConfig.m_ca_location.empty() &&
		...
		throw std::invalid_argument("OIDC dex_ca_path does not exist");
}
...
Principal OidcTokenVerifier::verify(const std::string &token)
{
	const auto decoded = [&token]()
	{
		...
		{
			throw std::domain_error("Dex bearer token is malformed");
		}
	}();
	if (!decoded.has_subject())
		throw std::domain_error("Dex bearer token is missing subject");
	if (!decoded.has_payload_claim("iss"))
		throw std::domain_error("Dex bearer token is missing issuer");
	if (!decoded.has_audience())
		throw std::domain_error("Dex bearer token is missing audience");
	if (!decoded.has_expires_at())
		throw std::domain_error("Dex bearer token is missing expiration");
	if (!decoded.has_header_claim("kid"))
		throw std::domain_error("Dex bearer token is missing key identifier");
	...
	{
		throw std::domain_error("Dex bearer token contains malformed claims");
	}
	if (m_config.allowedAlgorithms.count(algorithm) == 0)
		throw std::domain_error("Dex bearer token uses an unsupported signing algorithm");
	...
	{
		throw std::domain_error(Utility::stringFormat("Dex bearer token validation failed: %s", e.what()));
	}

	if (audiences.count(m_config.resourceAudience) == 0)
	{
		throw std::domain_error("Dex bearer token is not intended for the App Mesh API");
	}
...
void OidcTokenVerifier::refreshDiscoveryLocked()
{
	...
	if (!metadata.contains("issuer") || normalizeIssuer(metadata.at("issuer").get<std::string>()) != m_config.issuer)
		throw std::domain_error("OIDC discovery issuer does not match the configured Dex issuer");
...
std::string OidcTokenVerifier::resolveKey(const std::string &kid)
{
	...
	if (kid.empty())
		throw std::domain_error("Dex bearer token has an empty key identifier");
	if (kid.size() > MAX_KEY_ID_LENGTH)
		throw std::domain_error("Dex bearer token key identifier is too long");
	...
	if (negative != m_negativeKeys.end() && now - negative->second < NEGATIVE_KEY_TTL)
		throw std::domain_error("Dex bearer token references an unknown signing key");
		...
		now - m_keyRefreshAttemptedAt < KEY_REFRESH_MIN_INTERVAL)
	{
		...
		if (existing != m_keys.end())
			return existing->second.pem;
		if (m_keys.empty())
			throw AuthenticationUnavailableException("Dex discovery/JWKS refresh is temporarily rate-limited");
		throw std::domain_error("Dex bearer token references an unknown signing key");
	}
	...
	catch (const std::exception &e)
	{
		...
		if (existing != m_keys.end())
		{
			LOG_WAR << fname << "using cached Dex key after refresh failure: " << e.what();
			return existing->second.pem;
		}
		throw AuthenticationUnavailableException(
			Utility::stringFormat("Dex discovery/JWKS is unavailable: %s", e.what()));
	}
...
std::string OidcTokenVerifier::transportUrl(const std::string &publishedUrl) const
{
	...
	if (published.scheme != issuer.scheme || published.host != issuer.host || published.port != issuer.port)
		throw std::domain_error("OIDC metadata published an endpoint outside the configured Dex issuer");
	...
	if (issuerPath != "/" && published.path != issuerPath && !Utility::startWith(published.path, issuerPath + "/"))
		throw std::domain_error("OIDC metadata published an endpoint outside the configured Dex issuer path");
...
std::string OidcTokenVerifier::jwkToPem(const nlohmann::json &jwk)
{
	...
	if (!jwk.contains("kty") || jwk.at("kty") != "RSA" || !jwk.contains("n") || !jwk.contains("e"))
		throw std::invalid_argument("only RSA signing keys are supported in Dex JWKS");
