...
namespace
{
	...
	const char *FIRST_ADMIN_ENROLLMENT_FILE = "first-admin-enrollment-token";
	constexpr size_t FIRST_ADMIN_ENROLLMENT_TOKEN_BYTES = 32;
	...
	std::string generateEnrollmentToken()
	{
		unsigned char random[FIRST_ADMIN_ENROLLMENT_TOKEN_BYTES];
		if (RAND_bytes(random, sizeof(random)) != 1)
			throw std::runtime_error("failed to generate first-admin enrollment token");
...
std::shared_ptr<AuthorizationPrincipal> AuthorizationStore::enrollFirstAdmin(
	const Principal &principal, const std::string &enrollmentToken)
{
	const static char fname[] = "AuthorizationStore::enrollFirstAdmin() ";
	std::lock_guard<std::recursive_mutex> guard(m_mutex);
	if (!m_firstAdminEnrollmentEnabled || m_firstAdminEnrolled || hasFirstAdminLocked() ||
		m_firstAdminEnrollmentToken.empty())
	{
		closeFirstAdminEnrollmentLocked();
		throw AuthorizationException("first-admin enrollment is not available");
	}
	if (!Utility::secureCompare(m_firstAdminEnrollmentToken, enrollmentToken))
...
void AuthorizationStore::initializeFirstAdminEnrollment()
{
	...
	std::lock_guard<std::recursive_mutex> guard(m_mutex);
	m_firstAdminEnrollmentTokenPath = (fs::path(Configuration::instance()->getWorkDir()) /
		"auth" / "secrets" / FIRST_ADMIN_ENROLLMENT_FILE).string();
...
#else
	m_firstAdminEnrollmentEnabled = false;
#endif
	...
	const auto enrollmentDirectory = fs::path(m_firstAdminEnrollmentTokenPath).parent_path();
	if (!Utility::createDirectory(enrollmentDirectory.string(), fs::perms::owner_all))
		throw std::runtime_error("failed to create first-admin enrollment directory");
	if (!os::fileChmod(enrollmentDirectory.string(), 0700))
		throw std::runtime_error("failed to secure first-admin enrollment directory");

	m_firstAdminEnrollmentToken = generateEnrollmentToken();
	const auto tmp = os::createTmpFile(m_firstAdminEnrollmentTokenPath,
		m_firstAdminEnrollmentToken + "\n", 0600);
	if (tmp.empty())
		throw std::runtime_error("failed to create first-admin enrollment token file");
	if (ACE_OS::rename(tmp.c_str(), m_firstAdminEnrollmentTokenPath.c_str()) != 0)
	{
		Utility::removeFile(tmp);
		throw std::runtime_error(std::string("failed to publish first-admin enrollment token: ") + last_error_msg());
	}
	LOG_INF << fname << "first-admin enrollment is available only from loopback";
}
...
void AuthorizationStore::closeFirstAdminEnrollmentLocked()
{
	if (!m_firstAdminEnrollmentToken.empty())
	{
		OPENSSL_cleanse(&m_firstAdminEnrollmentToken[0], m_firstAdminEnrollmentToken.size());
		m_firstAdminEnrollmentToken.clear();
	}
	if (!m_firstAdminEnrollmentTokenPath.empty() && Utility::isFileExist(m_firstAdminEnrollmentTokenPath))
		Utility::removeFile(m_firstAdminEnrollmentTokenPath);
}
