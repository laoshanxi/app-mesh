...
void Ldap::ModStr::Clear(void)
{
    if (val.mod_values)
    {
        for (char **sval = val.mod_values; sval && *sval; ++sval)
            delete[] * sval;
        delete[] val.mod_values;
        val.mod_values = NULL;
    }
...
void Ldap::ModBin::Clear(void)
{
    if (val.mod_bvalues)
    {
        for (berval **bval = val.mod_bvalues; bval && *bval; ++bval)
            BervalDelete(*bval);
        delete[] val.mod_bvalues;
        val.mod_bvalues = NULL;
    }
...
std::string Ldap::ModBin::GetStringValue(void) const
{
    ...
    if (val.mod_bvalues && val.mod_bvalues[0])
    {
        ...
        if (ptr->bv_len && ptr->bv_val)
            res.assign(ptr->bv_val, ptr->bv_len);
    }
...
std::vector<char> Ldap::ModBin::GetBinaryValue(void) const
{
    ...
    if (val.mod_bvalues && val.mod_bvalues[0])
    {
        const berval *ptr = val.mod_bvalues[0];
        if (ptr->bv_len && ptr->bv_val)
            res.assign(ptr->bv_val, ptr->bv_val + ptr->bv_len);
    }
    ...
    std::vector<char> decode(const char *src, size_t srcsz)
    {
        ...
        if (0 < srcsz && srcsz % 4 == 0)
        {
            ...
            for (size_t ii = 0; ii < srcsz; ii += 4)
            {
                ...
                if (dst.size() < dstsz)
                    dst.push_back((triple >> 16) & 0xFF);
                if (dst.size() < dstsz)
                    dst.push_back((triple >> 8) & 0xFF);
                if (dst.size() < dstsz)
                    dst.push_back(triple & 0xFF);
            }
        }
        else
        {
            std::cerr << "Tools::base64Decode: "
                      << "incorrect size buf: " << srcsz;
        }
