...
void AppMeshDaemon::initializeRestService()
{
	...
	TcpHandler::initClientSSL(Global::getClientSSLContext(), "", "", clientCA);
	...
	m_client = TcpStreamPtr(new SocketStream(Global::getClientSSLContext()));
	if (m_client->connect(tcpAddr))
...
bool AppMeshDaemon::checkTcpConnection(int &errorCounter)
{
	...
	if (!m_client->connected())
	{
		...
		if (m_client->connect(acceptorAddr))
