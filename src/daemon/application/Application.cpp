...
// C++11 requires an out-of-class definition when this static constexpr member
// is ODR-used (for example by the startup-phase maps below).
constexpr int Application::DEFAULT_STARTUP_PHASE;
...
Application::Application()
	: m_kind(Kind::Managed), m_startupPhase(DEFAULT_STARTUP_PHASE), m_ownerPermission(0), m_metadata(EMPTY_STR_JSON),
...
void Application::FromJson(const std::shared_ptr<Application> &app, const nlohmann::json &jsonObj)
{
	...
	if (HAS_JSON_FIELD(jsonObj, JSON_KEY_APP_owner))
		throw std::invalid_argument(
			...
			"provision the corresponding principal and replace owner with owner_principal_id");
	...
	if (!startupPhase.empty())
	{
		static const std::map<std::string, int> phases = {
			{"auth-issuer", 20},
			{"ingress", 30}, {"normal", DEFAULT_STARTUP_PHASE}};
	...
	if (HAS_JSON_FIELD(jsonObj, JSON_KEY_APP_env))
	{
		...
		for (auto &env : envs.items())
		{
			if (!env.value().is_string())
				throw std::invalid_argument(Utility::stringFormat("env %s must be a string (quote bool/number-like values in YAML)", env.key().c_str()));
