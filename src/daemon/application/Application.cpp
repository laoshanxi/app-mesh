...
struct Application::Runtime
{
	...
	// Lock order: Application::m_process -> completionMutex -> runMutex -> Application::m_task.
	...
	std::map<RunCompletionSubscription, std::pair<std::string, RunCompletionCallback>> completionCallbacks;
...
bool Application::attach(int pid)
{
	...
	// scheduler forever. Resolve the start only after the identity probe.
	...
	// 3. Publish the recovered run-state and schedule intent as one lifecycle decision.
	{
		std::lock_guard<std::mutex> lifecycleGuard(m_runtime->lifecycleMutex);
		std::lock_guard<std::mutex> completionGuard(m_runtime->completionMutex);
...
void Application::maintainRuntime(const std::chrono::system_clock::time_point &now)
{
	std::shared_ptr<AppProcess> exitedProcess;
	{
		...
		const bool lacksReactorExitCallback = process && (process->isRecovered() || !process->containerId().empty());
		const bool canReportExit = process && process->canReportExit();
		const bool hasValidPid = pid > 1;
		if (lacksReactorExitCallback && canReportExit && hasValidPid && !process->running())
	...
	const std::string &processUuid, RunCompletionCallback callback)
{
	...
	std::lock_guard<std::mutex> guard(m_runtime->completionMutex);
	const auto run = m_runtime->load();
	if (run.id.empty() || (!processUuid.empty() && processUuid != run.id) ||
		run.phase == Runtime::Run::Phase::Idle || run.phase == Runtime::Run::Phase::Completed)
	{
		return INVALID_RUN_COMPLETION_SUBSCRIPTION;
	}

	const auto subscription = m_runtime->nextCompletionSubscription++;
	m_runtime->completionCallbacks.emplace(subscription, std::make_pair(run.id, std::move(callback)));
	return subscription;
}
...
void Application::unsubscribeRunCompletion(RunCompletionSubscription subscription)
{
	if (subscription == INVALID_RUN_COMPLETION_SUBSCRIPTION)
		return;
	std::lock_guard<std::mutex> guard(m_runtime->completionMutex);
	m_runtime->completionCallbacks.erase(subscription);
}
...
bool Application::onRemovalTimer()
{
	const static char fname[] = "Application::onRemovalTimer() ";
...
{
	std::map<RunCompletionSubscription, RunCompletionCallback> callbacks;
	{
		std::lock_guard<std::mutex> guard(m_runtime->completionMutex);
		m_runtime->update([&](Runtime::Run &run)
						  {
		...
		{
			if (it->second.first == runId)
			{
				callbacks.emplace(it->first, std::move(it->second.second));
				it = m_runtime->completionCallbacks.erase(it);
			}
			else
			{
				++it;
...
void Application::terminate(std::shared_ptr<AppProcess> p)
{
	if (p)
	{
		...
		if (p->isStartAccepted() && p->lastPid() > 1)
			p->onExit(AppProcess::FORCED_TERMINATION_EXIT_CODE);
	}
	...
	}
	{
		...
		if (process && process->blocksStart())
		{
			const bool replaceRecurringRun =
				isRecurring() && process->isStartAccepted() && process->canReportExit();
...
void Application::scheduleRemoval(int timeoutSeconds)
{
	...
	const auto timerId = this->registerTimer(1000L * timeoutSeconds, 0, fname, std::bind(&Application::onRemovalTimer, this));
	m_timerRemoveId.store(timerId, std::memory_order_release);
	if (!isValidTimerId(timerId))
		LOG_CRT << fname << "FATAL: failed to schedule removal for application <" << getName() << ">";
}
