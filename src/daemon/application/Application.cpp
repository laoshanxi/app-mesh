...
void Application::handleScheduling(const std::chrono::system_clock::time_point &now)
{
	...
	// apps self-arm their next run in onTimerSpawn, cron apps re-arm via handleError's RESTART,
...
bool Application::onTimerSpawn(std::uint64_t gen)
{
	const static char fname[] = "Application::onTimerSpawn() ";

	// Run only as the current generation: any newer arm or cancel (disable/destroy) supersedes us.
	if (gen != m_spawnGen.load())
	{
		LOG_WAR << fname << "Application <" << m_name << "> spawn superseded/cancelled, skip.";
		return false;
	}
	...
	// 4. Schedule the next periodic run. Reaching here with a current generation means no
	//    newer arm has happened, so we are the one to arm the next interval.
...
void Application::disable()
{
	...
	++m_spawnGen;						 // invalidate any in-flight spawn (correctness)
	this->cancelTimer(m_nextStartTimerId); // best-effort: drop the armed timer promptly
...
std::string Application::runApp(int timeoutSeconds)
{
	...
	// New run begins: publish a clean Running state (see onTimerSpawn for rationale).
	updateRunState([&](RunState &r) {
		r.startTime = boost::make_shared<std::chrono::system_clock::time_point>(std::chrono::system_clock::now());
		r.returnCode = INVALID_RETURN_CODE;
		r.exitTime = nullptr;
...
void Application::handleError()
{
	...
	case AppBehavior::Action::KEEPALIVE:
		...
		armSpawn(std::chrono::system_clock::now() + restartDelay());
...
void Application::armSpawn(const std::chrono::system_clock::time_point &when)
{
	const static char fname[] = "Application::armSpawn() ";

	nextLaunchTime(when);
	m_needsSchedule.store(false);
	const auto delay = std::chrono::duration_cast<std::chrono::milliseconds>(when - std::chrono::system_clock::now()).count();
	const auto timerId = this->registerTimer(delay, 0, fname, std::bind(&Application::onTimerSpawn, this, ++m_spawnGen));
	m_nextStartTimerId.store(timerId); // kept only for best-effort cancel
	if (!IS_VALID_TIMER_ID(timerId))
	{
		m_needsSchedule.store(true); // arm failed: tick retries
	}
	LOG_DBG << fname << "Next start for <" << m_name << "> is " << DateTime::formatLocalTime(when) << " timer <" << timerId << ">";
}
