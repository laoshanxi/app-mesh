...
Application::Application()
	: m_persistAble(true), m_status(STATUS::ENABLED), m_ownerPermission(0), m_shellApp(false),
	  ...
	  m_nextStartTimerId(INVALID_TIMER_ID), m_health(true), m_appId(Utility::createUUID()),
	  m_version(0), m_suicideTimerId(INVALID_TIMER_ID), m_starts(std::make_shared<prometheus::Counter>())
{
...
bool Application::attach(int pid)
{
	...
	if (pid > 1)
	{
		...
		m_pid = boost::make_shared<pid_t>(m_process->getpid());
...
void Application::execute(void *ptree)
{
	...
	refresh(ptree);
	if (m_procExitTime && *m_procExitTime > *m_procStartTime && m_return && nextRunTime == nullptr && getStatus() == STATUS::ENABLED)
	{
		// error handling
		onExit();
	}
...
void Application::spawn()
{
	...
	if (this->isEnabled())
	{
		...
		m_pid = boost::make_shared<pid_t>(m_process->spawnProcess(getCmdLine(), execUser, m_workdir, getMergedEnvMap(), m_resourceLimit, m_stdoutFile, m_metadata, APP_STD_OUT_MAX_FILE_SIZE, sudoSwitchUser));
		if (*m_pid > 0)
			checkProcStdoutFile = m_process;
...
std::shared_ptr<AppProcess> Application::allocProcess(bool monitorProcess, const std::string &dockerImage, const std::string &appName)
{
	...
	else
	{
		if (monitorProcess)
		{
			process.reset(new MonitoredProcess(appName));
		}
		else
		{
			process.reset(new AppProcess(appName));
		}
	}
	return process;
}
...
void Application::handleExit(int code)
{
	m_return = boost::make_shared<int>(code);
	m_pid.reset();
	...
	// this->registerTimer(0, 0, std::bind(&Application::onExit, this), fname);
}
...
void Application::onExit()
{
	...
	switch (this->exitAction(*m_return))
	{
