...
const std::string RestBase::generateJwtToken(const std::string &userName, const std::string &userGroup, const std::string &audience, int timeoutSeconds)
{
    ...
    auto jwt = jwt::create()
                   .set_type(HTTP_HEADER_JWT)                                 //.set_id(Utility::createUUID()) // JWT ID: prevents replay attacks
                   .set_issuer(Configuration::instance()->getRestJwtIssuer()) // Issuer: identifies token creator
                   .set_subject(userName)                                     // Subject: user ID
                   .set_audience(targetAudience)                              // Audience: intended recipient
...
const std::tuple<std::string, std::string, std::set<std::string>> RestBase::verifyToken(const HttpRequest &message, const std::string &audience)
{
    ...
    if (decoded_token.has_payload_claim("resource_access"))
    {
        auto resource_access = decoded_token.get_payload_claim("resource_access").to_json();
...
const std::string RestBase::getJwtUserName(const HttpRequest &message)
{
    ...
    if (Configuration::instance()->getJwt()->getJwtInterface() == JSON_KEY_USER_key_method_oauth2)
    {
        return std::get<0>(this->extractUserInfo(decoded_token));
    }
...
bool RestBase::permissionCheck(const HttpRequest &message, const std::string &permission, const std::string &audience)
{
    ...
    const auto roles = std::get<2>(result);

    std::set<std::string> permissions;
    for (const auto &role : roles)
    {
        const auto roleObj = Security::instance()->getRole(role);
        const auto rolePermissions = roleObj->getPermissions();
        permissions.insert(rolePermissions.begin(), rolePermissions.end());
    }

    if (permissions.count(permission))
    {
        LOG_DBG << fname << "authentication success for remote: " << message.m_remote_address << " with user: " << userName << " permission: " << permission;
        return true;
    }
    else
    {
    ...
    // TODO: realtime permission check follow "Keycloak Authorization API"
    // curl -H "Authorization: Bearer {access_token}" \
    //   -X POST "http://localhost:8080/auth/realms/appmesh-realm/protocol/openid-connect/token/introspect"
}
...
const std::tuple<std::string, std::string, std::set<std::string>> RestBase::extractUserInfo(jwt::decoded_jwt<jwt::traits::nlohmann_json> decoded)
{
...
const std::tuple<std::string, std::string, std::set<std::string>> RestBase::verifyKeycloakToken(const std::string &token,
