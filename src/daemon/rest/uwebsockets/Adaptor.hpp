...
private:
    ...
    void setupHandlers()
    {
        ...
        m_server->onWSOpen([this](auto connection)
        {
            LOG_DBG << "WebSocketAdaptor::onWSOpen() New WebSocket connection: " << connection->getId();
            std::lock_guard<std::mutex> lock(m_connMapMutex);
            m_connIdMap[connection->getId()] = connection->getNumericId();
        });

        // WebSocket: Handle connection close
        m_server->onWSClose([this](const std::string &connId, int code, std::string_view /*message*/)
        {
            LOG_DBG << "WebSocketAdaptor::onWSClose() Connection " << connId << " closed with code: " << code;
            uint64_t numericId = 0;
            {
                std::lock_guard<std::mutex> lock(m_connMapMutex);
                auto it = m_connIdMap.find(connId);
                if (it != m_connIdMap.end())
                {
                    numericId = it->second;
                    m_connIdMap.erase(it);
                }
            }
            if (numericId > 0)
            {
...
private:
    ...
    std::mutex m_connMapMutex;
    std::unordered_map<std::string, uint64_t> m_connIdMap;
};
