...
const std::shared_ptr<cpr::Response> DockerApiProcess::requestDocker(const web::http::method &mtd, const std::string &path, std::map<std::string, std::string> query, std::map<std::string, std::string> header, web::json::value *body)
{
	...
	try
	{
		...
		if (mtd == "GET")
		{
			*response = cpr::Get(cpr::Url{restURL, path}, cprHeader, cprParam);
		}
		else if (mtd == "POST")
		{
			*response = cpr::Post(cpr::Url{restURL, path}, cprHeader, cprParam, cprBody);
		}
		else if (mtd == "PUT")
		{
			*response = cpr::Put(cpr::Url{restURL, path}, cprHeader, cprParam, cprBody);
		}
		else if (mtd == "DELETE")
		{
			*response = cpr::Delete(cpr::Url{restURL, path}, cprHeader, cprParam);
		}
		return response;
	}
