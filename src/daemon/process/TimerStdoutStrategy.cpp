...
void TimerStdoutStrategy::startTimer(TimerHandler &owner, std::string processUuid)
{
	const static char fname[] = "TimerStdoutStrategy::startTimer() ";
	auto state = m_state;
	std::lock_guard<std::mutex> registrationGuard(state->registrationMutex);
	state->processUuid = std::move(processUuid);
	state->stopped.store(false, std::memory_order_release);
	state->timerId = owner.registerTimer(0, 1000, fname, [state]() { return onTimerDispatch(state); });
