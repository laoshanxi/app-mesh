...
std::shared_ptr<cpr::Response> ConsulConnection::requestConsul(const web::http::method &mtd, const std::string &path, std::map<std::string, std::string> query, std::map<std::string, std::string> header, web::json::value *body)
{
	...
	try
	{
		...
		{
			if (useAuth)
				*response = cpr::Get(cpr::Url{restURL, path}, sslOpts, cprHeader, cprParam, auth);
			else
				*response = cpr::Get(cpr::Url{restURL, path}, sslOpts, cprHeader, cprParam);
		}
		else if (mtd == "POST")
		{
			if (useAuth)
				*response = cpr::Post(cpr::Url{restURL, path}, sslOpts, cprHeader, cprParam, cprBody, auth);
			else
				*response = cpr::Post(cpr::Url{restURL, path}, sslOpts, cprHeader, cprParam, cprBody);
		}
		else if (mtd == "PUT")
		{
			if (useAuth)
				*response = cpr::Put(cpr::Url{restURL, path}, sslOpts, cprHeader, cprParam, cprBody, auth);
			else
				*response = cpr::Put(cpr::Url{restURL, path}, sslOpts, cprHeader, cprParam, cprBody);
		}
		else if (mtd == "DELETE")
		{
			if (useAuth)
				*response = cpr::Delete(cpr::Url{restURL, path}, sslOpts, cprHeader, cprParam, auth);
			else
				*response = cpr::Delete(cpr::Url{restURL, path}, sslOpts, cprHeader, cprParam);
		}
...
{
	const char *argv[] = {"appc", "-h"};
	ArgumentParser parser(2, argv);
	auto admin = Security::instance()->getUserInfo(JWT_ADMIN_NAME);
	parser.requestToken(admin->getName(), admin->getKey(), "", baseUri.to_string());
	return parser.requestHttp(false, mtd, requestPath, nullptr, headers, query);
}
