...
func runSubWorkflow(ctx context.Context, client *appmesh.AppMeshClient, wfName string, inputs map[string]string, depth int, opts Options) (int, map[string]string) {
	...
	// Drop OnJobDone / OnStepDone / StepLogPathFn / ActiveSteps because each
	// closes over the parent's wfName+runID at registration time and would
	// route sub-workflow checkpoint/log state into the parent's files. Log is
	// kept — its messages embed the sub's runID, so they're identifiable even
	// when written to the parent's flow.log.
