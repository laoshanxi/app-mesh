...
#[cfg(test)]
mod tests {
    use appmesh::{AppMeshClient, ClientConfig};
    ...
    async fn test_tags() {
        ...
        server.mock("PUT", "/appmesh/label/env").with_status(200).create_async().await;
        server.mock("GET", "/appmesh/labels").with_status(200).with_body(r#"{"env": "prod"}"#).create_async().await;
