...
pub trait ResponseExt {
    ...
    /// Reads the response body as text with a specific charset.
    fn text_with_charset(self, charset: &str) -> Result<String, AppMeshError>;
...
impl ResponseExt for http::Response<Bytes> {
    ...
    fn text(self) -> Result<String, AppMeshError> {
        ...
        if cfg!(target_os = "windows") && status == http::StatusCode::OK && is_utf8_text_content(&headers) {
    ...
    fn text_with_charset(self, charset_name: &str) -> Result<String, AppMeshError> {
        let body = self.into_body();

        // Get the encoding from encoding_rs
        let encoding = Encoding::for_label(charset_name.as_bytes()).unwrap_or(UTF_8);

        decode_with_charset(&body, Some(encoding))
    }
...
fn get_local_encoding() -> Option<&'static Encoding> {
    // On non-Windows platforms, check environment variables
    std::env::var("LC_ALL").or_else(|_| std::env::var("LC_CTYPE")).or_else(|_| std::env::var("LANG")).ok().and_then(
        |locale| {
            // Extract encoding from locale string (e.g., "zh_CN.GBK" -> "GBK")
            locale.split('.').nth(1).and_then(|enc| Encoding::for_label(enc.as_bytes()))
        },
    )
}
