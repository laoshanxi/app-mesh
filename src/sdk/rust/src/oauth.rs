//! Standards-based OAuth 2.0/OpenID Connect support for Dex.
//!
//! Authentication and token lifecycle operations go directly to Dex. App Mesh is a
...
/// Canonical Dex issuer and the independently selected network route used by this
    ...
    fn from_response(value: &Value, previous_refresh_token: Option<String>) -> Result<Self> {
        let access_token = value
            ...
            .ok_or_else(|| {
                AppMeshError::AuthenticationFailed(
                    "Dex token response did not include an access_token".into(),
                )
            })?
            .to_string();
        ...
        if !token_type.eq_ignore_ascii_case("bearer") {
            return Err(AppMeshError::AuthenticationFailed(format!(
                "Dex returned unsupported token type '{}'",
                token_type
            )));
        }
...
/// Discovered Dex OAuth client and refresh-capable token provider.
...
impl std::fmt::Debug for DexOAuthClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DexOAuthClient")
...
impl DexOAuthClient {
    /// Discover Dex with system trust roots and a bounded HTTP timeout.
    pub async fn discover(config: DexOAuthConfig) -> Result<Self> {
    ...
    /// Discover Dex using a caller-provided HTTP client. This is the supported hook
    /// for private CAs, proxies, and application-specific timeouts.
    pub async fn with_http_client(config: DexOAuthConfig, http: reqwest::Client) -> Result<Self> {
        ...
        if !status.is_success() {
            return Err(oauth_http_error("Dex discovery", status, &response.bytes().await?));
        }
        let metadata: OidcMetadata = response.json().await?;
        // Both sides are trailing-slash normalized: the config side by validate_and_normalize(),
        // the metadata side here, so a Dex that publishes "https://host/" matches "https://host".
        if metadata.issuer.trim_end_matches('/') != config.issuer {
            return Err(AppMeshError::AuthenticationFailed(
                "Dex discovery issuer does not match the configured issuer".into(),
            ));
        }
    ...
    pub fn config(&self) -> &DexOAuthConfig {
        &self.config
    }
    ...
    /// this synchronously after every successful Dex token response; applications
    ...
    pub fn authorization_request(&self, redirect_uri: &str) -> Result<AuthorizationRequest> {
        ...
        // published by Dex; access_url is only for SDK back-channel HTTP requests.
    ...
    pub async fn request_device_authorization(&self) -> Result<DeviceAuthorization> {
        let endpoint = self.metadata.device_authorization_endpoint.as_deref().ok_or_else(|| {
            AppMeshError::UnsupportedFeature {
                ...
                transport: "Dex discovery metadata".into(),
            }
        })?;
        let scope = self.user_scope();
        let response = self
        ...
        let verification_uri = device
            ...
            .ok_or_else(|| {
                AppMeshError::AuthenticationFailed(
                    "Dex device response did not include a verification_uri".into(),
                )
            })?;
        ...
        if device.device_code.is_empty() || device.user_code.is_empty() {
            return Err(AppMeshError::AuthenticationFailed(
                "Dex returned an incomplete device authorization response".into(),
            ));
        }
    ...
    pub async fn refresh(&self) -> Result<TokenSet> {
        ...
        let previous = self.snapshot_tokens().ok_or_else(|| {
            AppMeshError::AuthenticationFailed("no Dex token set is available".into())
        })?;
        let refresh_token = previous.refresh_token.clone().ok_or_else(|| {
            AppMeshError::AuthenticationFailed("no Dex refresh token is available".into())
        })?;
        let response = self
            .post_form(
                &self.metadata.token_endpoint,
    ...
    /// Revoke refresh and access tokens directly at Dex. Returns `false` when Dex does
    /// not advertise RFC 7009 revocation or when no token set is held (in both cases
    /// nothing was revoked at Dex). Provider-local state is cleared on every path,
        ...
        let result = async {
            ...
            for (token, hint) in [
                ...
                if !response.status().is_success() {
                    let status = response.status();
                    return Err(oauth_http_error("Dex token revocation", status, &response.bytes().await?));
                }
            }
            Ok(true)
        }
    ...
    pub async fn userinfo(&self) -> Result<Value> {
        ...
        let status = response.status();
        if !status.is_success() {
            return Err(oauth_http_error("Dex userinfo", status, &response.bytes().await?));
        }
    ...
    /// Install a fresh Dex token response. The token set is stored in memory first,
    ...
    fn access_endpoint(&self, published: &str) -> Result<String> {
        ...
        let suffix = published.path().strip_prefix(issuer_path).ok_or_else(|| {
            AppMeshError::AuthenticationFailed("Dex endpoint is outside the configured issuer path".into())
        })?;
    ...
    async fn fetch_jwks(&self) -> Result<JsonWebKeySet> {
        ...
        let status = response.status();
        if !status.is_success() {
            return Err(oauth_http_error("Dex JWKS request", status, &response.bytes().await?));
        }
        let jwks: JsonWebKeySet = response.json().await?;
        if jwks.keys.is_empty() {
            return Err(AppMeshError::AuthenticationFailed("Dex JWKS contains no keys".into()));
        }
        Ok(jwks)
    }
    ...
    async fn validate_id_token(&self, response: &Value, expected_nonce: &str) -> Result<()> {
        let id_token = response.get("id_token").and_then(Value::as_str).ok_or_else(|| {
            AppMeshError::AuthenticationFailed(
                "Dex authorization response did not include an ID token".into(),
            )
        })?;
        let (header, claims, signing_input, signature) = decode_jwt(id_token)?;
        if header.alg != "RS256" || header.kid.is_empty() {
            return Err(AppMeshError::AuthenticationFailed(
                "Dex ID token must use RS256 with a kid".into(),
            ));
        }
        ...
        let jwk = jwk.ok_or_else(|| {
            AppMeshError::AuthenticationFailed("Dex ID token kid is not present in JWKS".into())
        })?;
        if jwk.kty != "RSA"
        ...
        {
            return Err(AppMeshError::AuthenticationFailed(
                "Dex JWKS key is not an RS256 signing key".into(),
            ));
        }
        let modulus = URL_SAFE_NO_PAD.decode(
            jwk.n
                .as_deref()
                .ok_or_else(|| {
                    AppMeshError::AuthenticationFailed("Dex RSA signing key has no modulus".into())
                })?
                .as_bytes(),
        )?;
        let exponent = URL_SAFE_NO_PAD.decode(
            jwk.e
                .as_deref()
                .ok_or_else(|| {
                    AppMeshError::AuthenticationFailed("Dex RSA signing key has no exponent".into())
                })?
                .as_bytes(),
        )?;
        RsaPublicKeyComponents { n: &modulus, e: &exponent }
            .verify(&RSA_PKCS1_2048_8192_SHA256, signing_input.as_bytes(), &signature)
            .map_err(|_| AppMeshError::AuthenticationFailed("Dex ID token signature is invalid".into()))?;
    ...
    async fn post_form(&self, published_endpoint: &str, form: &[(&str, &str)]) -> Result<Value> {
        ...
        let body = response.bytes().await?;
        if !status.is_success() {
            return Err(oauth_http_error("Dex OAuth request", status, &body));
        }
...
#[async_trait]
impl TokenProvider for DexOAuthClient {
    async fn access_token(&self) -> Result<String> {
        ...
        if tokens.needs_refresh(Duration::from_secs(DEFAULT_REFRESH_LEEWAY)) {
            ...
            if tokens.is_expired() {
                return Err(AppMeshError::AuthenticationFailed(
                    "Dex access token expired and no refresh token is available".into(),
                ));
            }
        }
    ...
    async fn force_refresh(&self) -> Result<TokenSet> {
        if self.snapshot_tokens().and_then(|tokens| tokens.refresh_token).is_some() {
            ...
            Err(AppMeshError::AuthenticationFailed(
                "the Dex token provider has no refresh grant".into(),
            ))
        }
    }
...
fn validate_published_endpoint(config: &DexOAuthConfig, endpoint: &str) -> Result<()> {
    let endpoint = Url::parse(endpoint).map_err(|error| {
        AppMeshError::AuthenticationFailed(format!("Dex published an invalid endpoint: {}", error))
    })?;
    ...
    {
        return Err(AppMeshError::AuthenticationFailed(
            "Dex published an endpoint outside the configured issuer origin".into(),
        ));
    }
    let issuer_path = issuer.path().trim_end_matches('/');
    ...
    {
        return Err(AppMeshError::AuthenticationFailed(
            "Dex published an endpoint outside the configured issuer path".into(),
        ));
    }
    Ok(())
}
...
fn decode_jwt(token: &str) -> Result<(JwtHeader, Value, String, Vec<u8>)> {
    ...
    if parts.len() != 3 || parts.iter().any(|part| part.is_empty()) {
        return Err(AppMeshError::AuthenticationFailed(
            "Dex returned a malformed ID token".into(),
        ));
    }
...
fn validate_id_token_claims(
    ...
    if claims.get("nonce").and_then(Value::as_str) != Some(expected_nonce) {
        return Err(AppMeshError::AuthenticationFailed("Dex ID token nonce mismatch".into()));
    }
    if claims.get("iss").and_then(Value::as_str) != Some(config.issuer.as_str()) {
        return Err(AppMeshError::AuthenticationFailed("Dex ID token issuer mismatch".into()));
    }
    ...
    if !audience_matches {
        return Err(AppMeshError::AuthenticationFailed("Dex ID token audience mismatch".into()));
    }
    if let Some(Value::Array(audiences)) = claims.get("aud") {
        ...
        {
            return Err(AppMeshError::AuthenticationFailed(
                "Dex ID token authorized-party mismatch".into(),
            ));
        }
    }
    let expiry = claims.get("exp").and_then(Value::as_u64).ok_or_else(|| {
        AppMeshError::AuthenticationFailed("Dex ID token has no valid expiration".into())
    })?;
    if expiry <= now_epoch_seconds() {
        return Err(AppMeshError::AuthenticationFailed("Dex ID token is expired".into()));
    }
    if claims
        .get("nbf")
        .and_then(Value::as_u64)
    ...
    {
        return Err(AppMeshError::AuthenticationFailed(
            "Dex ID token is not valid yet".into(),
        ));
    }
    Ok(())
}
