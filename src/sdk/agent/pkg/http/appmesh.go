...
var (
	...
	localConnection   *Connection // tcp connection to the server
...
func ListenRest() {
	...
	var err error
	localConnection, err = NewConnection(targetHost, false)
	if err != nil {
...
func handleAppmeshResquest(ctx *fasthttp.RequestCtx) {
	...
	if targetHost != "" {
		...
		if parsedURL.Port() == strconv.Itoa(config.ConfigData.REST.RestListenPort) {
			...
			var err error
			targetConnection, err = NewConnection(parsedURL.Host, true)
			if err != nil {
