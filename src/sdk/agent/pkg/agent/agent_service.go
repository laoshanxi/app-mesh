...
func MonitorConnectionResponse(conn *Connection, allowError bool) {
	// On exit the connection is dead: drop it from the pool and close it so any
	// waiters (including on dedicated connections) are released.
	defer func() {
	...
	for {
		...
		if err != nil {
			// Closed intentionally (e.g. a dedicated transfer connection done) — not an error.
			select {
			case <-conn.closed:
				return
			default:
...
func HandleAppMeshRequest(w http.ResponseWriter, r *http.Request) {
	...
	if forwardingHost != "" {
		...
		if forwardingURL.Port() == "" || forwardingURL.Port() == strconv.Itoa(config.ConfigData.REST.RestListenPort) {
			...
			return
		} else if r.URL.Path == REST_PATH_UPLOAD || r.URL.Path == REST_PATH_DOWNLOAD {
			// File transfers get a dedicated connection to isolate the daemon's
			// per-connection transfer state from other multiplexed requests.
			targetConnection, err = newDedicatedConnection(forwardingAddr, config.ConfigData.REST.SSL.VerifyServerDelegate)
			if err != nil {
				logger.Errorf("Failed to create dedicated connection to %s: %v", forwardingHost, err)
...
func HandleUpload(w http.ResponseWriter, r *http.Request, data *Response, filePath string) error {
	...
	if data.TempUploadFilePath == "" {
		...
		err := appmesh.ApplyFileAttributes(filePath, r.Header)
		if err != nil {
