...
func handleRestFile(w http.ResponseWriter, r *http.Request, data *Response) bool {
	...
	if r.Method == http.MethodGet && r.URL.Path == REST_PATH_DOWNLOAD && data.HttpStatus == http.StatusOK {
		if data.TempDownloadFilePath != "" {
			// Handle file which is transferred with delegation
		...
		if utils.IsValidFileName(filePath) {
			...
			http.Error(w, "Invalid file name", http.StatusNotAcceptable)
			return false
		}
	} else if r.Method == http.MethodPost && r.URL.Path == REST_PATH_UPLOAD && data.HttpStatus == http.StatusOK {
		// Handle upload file
		...
		if utils.IsValidFileName(filePath) {
			if err := saveHttpFile(w, r, filePath); err != nil {
				log.Printf("Save file failed with error: %v", err)
...
func saveHttpFile(w http.ResponseWriter, r *http.Request, filePath string) error {
	...
	// Parse the multipart form, with a reasonable max memory size for form parsing
	if err := r.ParseMultipartForm(10 << 20); err == nil { // 10MB limit for in-memory file storage; rest goes to disk
