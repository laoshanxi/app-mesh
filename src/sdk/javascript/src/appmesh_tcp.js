...
const HTTP_HEADER_KEY_X_TARGET_HOST = 'X-Target-Host'
...
class AppMeshClientTCP extends AppMeshClient {
  ...
  async _request (method, path, body = null, options = {}) {
    ...
    if (this.forwardingHost) {
      request.headers[HTTP_HEADER_KEY_X_TARGET_HOST] = this.forwardingHost.includes(':')
        ? this.forwardingHost
        : `${this.forwardingHost}:${this.tcpTransport.address[1]}`
    }
   ...
   * @param {boolean} [applyAttrs=false] - Apply returned mode and best-effort owner/group metadata
   * on non-Windows Node.js hosts
   * @override
   */
  async download_file (filePath, localFile = null, applyAttrs = false) {
