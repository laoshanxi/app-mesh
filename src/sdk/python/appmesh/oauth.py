...
Identity authentication happens directly at the authentication service. App Mesh receives
only the access token as an RFC 6750 bearer token. It never receives passwords, MFA
challenges, or directory-management requests.
"""
...
class OAuthError(AppMeshAuthError):
    """The authentication service rejected a request or returned an invalid response."""


class OAuthClient(TokenProvider):
    """Acquire tokens and attach their access token to an :class:`AppMeshClient`.
    ...
    def from_appmesh(
        ...
        **legacy_options,
        ...
        ``appmesh_client.base_url`` selects the Engine host. ``access_url`` selects
        ...
        return cls(
            ...
            access_url=access_url or config["issuer"],
    ...
    def _validate_ssl_verify(value: Union[bool, str]) -> Union[bool, str]:
        ...
        if isinstance(value, str) and value and os.path.exists(value):
            return value
        raise ValueError("ssl_verify must be true, false, or an existing CA path")

    def _discover(self) -> Dict[str, Any]:
        url = self.access_url + "/.well-known/openid-configuration"
        try:
            response = self.session.get(url, verify=self.ssl_verify, timeout=self.timeout)
            response.raise_for_status()
            metadata = response.json()
        except (requests.RequestException, ValueError) as exc:
            raise AppMeshRequestError("Authentication service discovery failed") from exc

        discovered_issuer = metadata.get("issuer")
        if discovered_issuer != self.issuer:
            raise OAuthError("The discovered issuer does not match the configured issuer")
        for endpoint in ("authorization_endpoint", "token_endpoint"):
            if not metadata.get(endpoint):
                raise OAuthError("Discovery metadata is missing " + endpoint)
            self._access_endpoint(metadata[endpoint])
        return metadata

    def _access_endpoint(self, published_url: str) -> str:
        """Map a canonical endpoint to the selected network address."""
        if not isinstance(published_url, str) or not published_url:
            raise OAuthError("Discovery metadata published an invalid endpoint")
        published = parse.urlsplit(published_url)
        issuer = parse.urlsplit(self.issuer)
        try:
            published_port = published.port
        except ValueError as exc:
            raise OAuthError("Discovery metadata published an endpoint with an invalid port") from exc
        if (
            ...
            raise OAuthError("Discovery metadata published an endpoint outside the configured issuer")
        ...
        if issuer_path and published.path != issuer_path and not published.path.startswith(issuer_path + "/"):
            raise OAuthError("The discovered endpoint is outside the configured issuer path")
        suffix = published.path[len(issuer_path):]
        target = self.access_url + suffix
        if published.query:
    ...
    @staticmethod
    def _oauth_error(response: requests.Response) -> OAuthError:
        try:
        ...
        try:
            with self._lock:
                if self.session is None:
                    raise AppMeshRequestError("OAuth client is closed")
            ...
            raise AppMeshRequestError("Token request failed") from exc
        if not response.ok:
            raise self._oauth_error(response)
        try:
            return response.json()
        except ValueError as exc:
            raise OAuthError("The authentication service returned a non-JSON token response", response.status_code) from exc
    ...
    def _install(
        ...
        if not isinstance(access_token, str) or not access_token:
            raise OAuthError("The token response did not include an access token")
        token_type = tokens.get("token_type", "Bearer")
        if not isinstance(token_type, str) or token_type.lower() != "bearer":
            raise OAuthError("The authentication service returned an unsupported access-token type")
    ...
    def _renew_locked(self) -> str:
        if self._tokens.get("refresh_token"):
            ...
            raise OAuthError("No credential is available to refresh the access token")
    ...
    def authorization_request(
        ...
        with self._lock:
            ...
            if state in self._pending_authorizations:
                raise OAuthError("An authorization request already uses this state")
    ...
    def complete_authorization_callback(
        ...
        if len(states) != 1 or not states[0]:
            raise OAuthError("Authorization callback is missing a single valid state")
        state = states[0]

        with self._lock:
            pending = self._pending_authorizations.pop(state, None)
        if pending is None:
            raise OAuthError("Authorization callback state is invalid or already consumed")
        if time.monotonic() - pending["created_at"] > self._AUTHORIZATION_REQUEST_LIFETIME:
            raise OAuthError("Authorization callback state has expired")
        ...
        if (callback.scheme, callback.netloc, callback.path) != (
            ...
            raise OAuthError("Authorization callback does not match the registered redirect URI")
        ...
        if len(codes) != 1 or not codes[0]:
            raise OAuthError("Authorization callback is missing a single code")

        expected_nonce = pending["nonce"]
        if expected_nonce is not None and id_token_validator is None:
            raise OAuthError("An OIDC ID-token validator is required for a nonce-bearing request")
        ...
        if expected_nonce is not None:
            ...
            if not isinstance(id_token, str) or not id_token:
                raise OAuthError("The token response did not include the nonce-bound ID token")
    ...
    def device_authorization(self, scopes: Optional[Iterable[str]] = None) -> Dict[str, Any]:
        ...
        if not endpoint:
            raise OAuthError("The authentication service does not advertise a device authorization endpoint")
        ...
        for key in ("verification_uri", "verification_uri_complete"):
            if device.get(key):
                # Validate the URL, but preserve the canonical public front-channel URL.
    ...
    def wait_for_device_authorization(
        ...
        """Poll until a device request is approved, denied, or expires."""
        if on_prompt:
        ...
        try:
            ...
            if endpoint:
                for hint in ("refresh_token", "access_token"):
                    ...
                    except (requests.RequestException, OAuthError, AttributeError):
                        success = False
            else:
                success = not tokens
        except (requests.RequestException, OAuthError, AttributeError):
    ...
    def close(self) -> None:
        """Close local HTTP resources. Call :meth:`revoke` to revoke tokens."""
        with self._lock:
            if self.session:
                self.session.close()
                self.session = None
