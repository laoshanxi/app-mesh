...
class AppMeshClient:
    ...
    with TLS transport and Dex bearer-token authentication.
    ...
    Example:
        ...
        >>> client = AppMeshClient(bearer_token="dex-access-token")
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            bearer_token: Dex access token to send as an RFC 6750 bearer token. Token acquisition,
              refresh, persistence, and revocation are handled by :class:`DexOAuthClient`.
            token_provider: Provider that supplies and refreshes Dex access tokens. Mutually
    ...
    def set_bearer_token(self, token: str) -> None:
        """Attach a caller-owned, non-refreshing Dex access token."""
        self._token_provider = StaticAccessTokenProvider(token)

    def clear_bearer_token(self) -> None:
        """Detach local authentication state without contacting Dex."""
        self._token_provider = None
    ...
    def forward_to(self) -> str:
        ...
        Notes:
            Every target node must trust the same Dex issuer and App Mesh resource audience.
    ...
    def get_auth_config(self) -> Dict[str, Any]:
        """Return the public Dex/OIDC configuration advertised by App Mesh."""
        resp = self._request_http(AppMeshClient._Method.GET, path="/appmesh/auth/config")
        return resp.json()

    def get_current_principal(self) -> Dict[str, Any]:
        """Return the verified Dex principal represented by the current bearer token."""
    ...
    def list_principals(self) -> Dict[str, Any]:
        """List App Mesh authorization overlays keyed by immutable Dex principal ID."""
    ...
    def update_principal(self, principal_id: str, principal_data: dict) -> None:
        """Create or update an App Mesh authorization overlay for a Dex principal."""
