...
Authentication is delegated to the configured authentication service. This process is an
OAuth resource server. It validates the access token and forwards the same token
...
Environment:
    ...
    APPMESH_AUTH_ISSUER      canonical issuer embedded in tokens (required)
    APPMESH_AUTH_ACCESS_URL  network route to the authentication service (required)
    APPMESH_AUTH_CA_PATH     CA file/dir for independent TLS verification (optional)
    APPMESH_AUTH_TLS_VERIFY  set "false" to disable TLS verification (development only)
    APPMESH_AUTH_AUDIENCE    App Mesh resource audience (default appmesh-api)
"""
...
def _client(token: AccessToken) -> AppMeshClient:
    """Build an App Mesh client bound to the caller's access token."""
...
_CURL_TOKEN_NOTE = (
    "Set APPMESH_TOKEN to an access token for the App Mesh API before running. "
...
# Principals / RBAC
# --------------------------------------------------------------------------- #
@mcp.tool(description="Get the current principal and its App Mesh authorization overlay.")
...
@mcp.tool(description="Create or update roles, status, and execution mapping for a principal.")
