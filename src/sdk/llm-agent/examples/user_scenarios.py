...
(--worker-app/--worker-session). The caller supplies a Dex bearer whose Engine
...
class LLMAgent:
    ...
    def with_bearer(self, bearer):
        """Attach a caller-obtained Dex access token; the agent is not an OAuth client."""
...
def main():
    ...
    if not bearer:
        p.error("APPMESH_BEARER_TOKEN is required; obtain it directly from Dex")
