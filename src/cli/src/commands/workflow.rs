...
async fn get(cli: &Cli, args: &WorkflowGetArgs) -> Result<i32> {
    let client = build_client_with_auth(cli).await?;
    let resp = client.run_task(WORKFLOW_TRIGGER_APP, with_token(&client, json!({"action": "workflow_get", "workflow": args.name})), 10).await.context(task_err("get"))?;
    let v = parse_resp(&resp)?;
...
async fn list(cli: &Cli) -> Result<i32> {
    let client = build_client_with_auth(cli).await?;
    let resp = client.run_task(WORKFLOW_TRIGGER_APP, with_token(&client, json!({"action": "workflow_list"})), 10).await.context(task_err("list"))?;
    let v = parse_resp(&resp)?;
...
async fn rm(cli: &Cli, args: &WorkflowRmArgs) -> Result<i32> {
    let client = build_client_with_auth(cli).await?;
    let resp = client.run_task(WORKFLOW_TRIGGER_APP, with_token(&client, json!({"action": "workflow_rm", "workflow": args.name})), 10).await.context(task_err("rm"))?;
    let v = parse_resp(&resp)?;
    eprintln!("{}", v["message"].as_str().unwrap_or("ok"));
    Ok(0)
}
...
async fn follow_run(client: &Arc<appmesh::AppMeshClientWSS>, workflow: &str, run_id: &str) -> Result<()> {
    ...
    loop {
        ...
        match client.run_task(WORKFLOW_TRIGGER_APP, with_token(client, json!({"action": "log", "workflow": workflow, "run_id": run_id})), 10).await {
            Ok(resp) => {
                iter_ok = true;
                if let Ok(v) = parse_resp(&resp) {
                    if let Some(log) = v["data"].as_str() {
                        // Slice by byte offset only at a valid char boundary; a
                        // multibyte char straddling log_offset would otherwise panic.
                        if let Some(delta) = log.get(log_offset..) {
                            if !delta.is_empty() {
                                print!("{}", delta);
                                log_offset = log.len();
                            }
                        } else if log.len() < log_offset {
                            // Log shrank/rotated unexpectedly — resync from the start.
                            print!("{}", log);
                            log_offset = log.len();
                        }
                    }
        ...
        match client.run_task(WORKFLOW_TRIGGER_APP, with_token(client, json!({"action": "run_detail", "workflow": workflow, "run_id": run_id})), 10).await {
            Ok(resp) => {
                iter_ok = true;
                if let Ok(v) = parse_resp(&resp) {
...
async fn runs(cli: &Cli, args: &WorkflowRunsArgs) -> Result<i32> {
    let client = build_client_with_auth(cli).await?;
    let resp = client.run_task(WORKFLOW_TRIGGER_APP, with_token(&client, json!({"action": "runs", "workflow": args.name})), 10).await.context(task_err("runs"))?;
    let v = parse_resp(&resp)?;
...
async fn logs(cli: &Cli, args: &WorkflowLogsArgs) -> Result<i32> {
    let client = build_client_with_auth(cli).await?;
    let resp = client.run_task(WORKFLOW_TRIGGER_APP, with_token(&client, json!({"action": "log", "workflow": args.workflow, "run_id": args.run_id})), 10).await.context(task_err("logs"))?;
    let v = parse_resp(&resp)?;
...
async fn output(cli: &Cli, args: &WorkflowOutputArgs) -> Result<i32> {
    let client = build_client_with_auth(cli).await?;
    let resp = client.run_task(WORKFLOW_TRIGGER_APP, with_token(&client, json!({"action": "step_log", "workflow": args.workflow, "run_id": args.run_id, "job": args.job, "step": args.step})), 10).await.context(task_err("output"))?;
    let v = parse_resp(&resp)?;
    if let Some(d) = v["data"].as_str() { print!("{}", d); }
    Ok(0)
}
...
async fn cancel(cli: &Cli, args: &WorkflowCancelArgs) -> Result<i32> {
    let client = build_client_with_auth(cli).await?;
    let resp = client.run_task(WORKFLOW_TRIGGER_APP, with_token(&client, json!({"action": "cancel", "workflow": args.workflow, "run_id": args.run_id})), 10).await.context(task_err("cancel"))?;
    let v = parse_resp(&resp)?;
    eprintln!("{}", v["message"].as_str().unwrap_or("ok"));
    Ok(0)
}
...
async fn rerun(cli: &Cli, args: &WorkflowRerunArgs) -> Result<i32> {
    let client = build_client_with_auth(cli).await?;
    let resp = client.run_task(WORKFLOW_TRIGGER_APP, with_token(&client, json!({"action": "rerun", "workflow": args.workflow, "run_id": args.run_id})), 30).await.context(task_err("rerun"))?;
    let v = parse_resp(&resp)?;
...
async fn detail(cli: &Cli, args: &WorkflowDetailArgs) -> Result<i32> {
    let client = build_client_with_auth(cli).await?;
    let resp = client.run_task(WORKFLOW_TRIGGER_APP, with_token(&client, json!({"action": "run_detail", "workflow": args.workflow, "run_id": args.run_id})), 10).await.context(task_err("detail"))?;
    let v = parse_resp(&resp)?;
...
async fn inputs(cli: &Cli, args: &WorkflowInputsArgs) -> Result<i32> {
    let client = build_client_with_auth(cli).await?;
    let resp = client.run_task(WORKFLOW_TRIGGER_APP, with_token(&client, json!({"action": "workflow_inputs", "workflow": args.name})), 10).await.context(task_err("inputs"))?;
    let v = parse_resp(&resp)?;
    let data = &v["data"];
