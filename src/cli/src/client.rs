...
use appmesh::{AppMeshClientWSS, ClientBuilderWSS, OAuthClient};
use std::sync::Arc;
...
pub async fn build_client(cli: &Cli) -> Result<Arc<AppMeshClientWSS>> {
    ...
    // An explicitly passed endpoint becomes the remembered default only after a
    // successful logon; a one-off or mistyped -H must not hijack later commands.
    if cli.host_url.is_none() {
        config::save_last_host(&host, wss_port);
    }
    Ok(client)
}

/// Persist the resolved endpoint as the CLI default. Called after a successful
/// logon, where the endpoint has proven reachable and authenticated.
pub fn remember_engine_endpoint(cli: &Cli) -> Result<()> {
    let (host, port, _) = resolve_address_and_tls(cli)?;
    config::save_last_host(&host, port);
    Ok(())
}
...
pub async fn build_client_with_auth(cli: &Cli) -> Result<Arc<AppMeshClientWSS>> {
    ...
    // Unattended automation (CI, containers) supplies an already-acquired
    ...
    // no refresh token and no local sign-in session, so `appm logon` remains the
    ...
    let session = config::load_session(&endpoint)?.ok_or_else(|| {
        anyhow::anyhow!("No sign-in session is configured for {}. Run 'appm logon'.", endpoint)
    })?;
        ...
        || advertised_audience != session.oauth.audience.as_deref()
    {
        ...
        anyhow::bail!(
            "The stored sign-in session does not match the authentication configuration for {}. The incompatible session was removed. Run 'appm logon' again.",
            endpoint
        );
    }
    ...
    let oauth = Arc::new(
        OAuthClient::discover(session.oauth.clone())
            .await
    ...
    oauth
        ...
        .await
        .context("Access token preparation failed")?;
