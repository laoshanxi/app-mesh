...
/**
 * Return the current user's home directory (ignore sudo).
 */
static std::string getCurrentUserHome()
{
#ifndef _WIN32
...
#ifdef _WIN32
	dir = fs::path(getCurrentUserHome()) / "AppMesh";
#else
	std::string home = getCurrentUserHome();
#ifdef __APPLE__
