...
Every workflow request carries the caller's Dex access token. The workflow engine submits
...
- Manual Run: use the validated caller's Dex access token; record the caller Principal ID
  as `actor`.
...
- Manual steps have exactly the caller's App Mesh authority and fail closed when the Dex
...
- Dex bearer tokens are ephemeral request material and cannot appear in durable run state.
