...
- Timer registration and cancellation may be called concurrently after main-thread singleton
  initialization. Expiration callbacks are deliberately serialized on the adapter's single thread;
  a blocking callback therefore delays other timers and is not an acceptable general work executor.
