...
This ADR records the v1 prototype decision. The current target design is the
unified Workflow Run model in ADR 0004: a long-lived engine consumes pending
Run records, and trigger sources create Runs through one API instead of calling
separate `run`/`serve` execution paths.
...
1. **Per-run process (Python)**: Each workflow execution spawns a separate Python process. A separate `wf-engine` process handles events and cron. Simple isolation but high overhead (~30-50MB and ~300ms startup per run) and requires Python runtime on target machines.
...
3. **Single Go process with goroutines**: One long-running `wf-engine serve` process handles event subscriptions, cron scheduling, and executes each workflow run as a goroutine (~8KB, ~1μs startup). A standalone `wf-engine run` mode exists for CLI ad-hoc execution and testing.

## Decision

**Option 3 — single Go binary, two modes:**

- `wf-engine serve` — long-running mode. Scans for workflow definitions, subscribes to App events, checks cron schedules, and runs triggered workflows as goroutines within the same process. Registered as an App Mesh App (`behavior.exit: restart`).
...
Both modes share the same Go packages (parser, expression, DAG, executor, engine). The `serve` mode additionally includes the trigger subsystem (registry, event listener, cron scheduler, concurrency manager).
