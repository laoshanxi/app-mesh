...
1. **Per-run process (Python)**: Each workflow execution spawns a separate Python process. A separate `workflow` process handles events. Simple isolation but high overhead (~30-50MB and ~300ms startup per run) and requires Python runtime on target machines.
...
- **Single binary, zero dependencies**: `go build` produces one `workflow` binary. No Python runtime, no pip packages, no virtualenv.
...
- **One process to manage**: App Mesh registers and monitors a single `workflow` App, not N+1 processes.
