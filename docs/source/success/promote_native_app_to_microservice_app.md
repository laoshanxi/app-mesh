...
App Mesh accepts only Dex access tokens. Acquire a token through the standard Dex
Authorization Code with PKCE, Device Authorization, or an approved service-client flow,
then provide it through the environment. The Dex issuer and the address used to reach Dex
may be configured independently; see [Security](../Security.md).

```shell
export APPMESH_BEARER_TOKEN="$(your-dex-oauth-client)"
```
...
With the Dex bearer, call the native app through App Mesh REST. The body can include the
