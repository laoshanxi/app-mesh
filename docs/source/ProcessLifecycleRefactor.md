...
| `terminate`/超时/disable/delete | 先标记 `terminating`，后端停止后合成 `onExit(9)`；disable/delete 同时终止 current 与 buffer；exit phase guard 与自然退出去重 | 原有 status/PID/exit time 收敛；不新增 SDK 或事件字段 |
...
flowchart LR
    ...
    PROC -->|onExit schedules| TIMER[TimerManager one-shot callback]
...
stateDiagram-v2
    [*] --> Pending
    Pending --> [*]: expected start failure; exit is Finalized
...
- 启动拒绝不保存额外 start 状态：`exitPhase == Finalized` 已能唯一表达“没有形成有效进程 run”，并直接唤醒等待者，不伪造 PROCESS_EXIT。
...
stateDiagram-v2
    [*] --> Active
    Active --> Observed: exit result published under lifecycle lock
    Observed --> Finalizing: start is Accepted; one-shot timer claimed
    Finalizing --> Finalized: resources and Application state committed
    Finalized --> [*]
```

`Active -> Observed` 是唯一的退出认领点。自然退出、主动终止、Docker polling、恢复进程 polling 即使同时到达，也只有一个路径能在 lifecycle lock 下发布结果；结果字段先写入，再以 release store 发布 `Observed`，避免 timer callback 读到旧 exit code。START gate 已打开时，同一临界区把状态推进到 `Finalizing`，随后在锁外登记 one-shot timer；重复退出观察不会产生第二个 timer。
...
stateDiagram-v2
    [*] --> Completed
    Completed --> Starting: new internal run id installed
...
### 3.4 lifecycle 相关字段与判断函数审计

`StartPhase` 与 `ExitPhase` 不合成一个大 enum：实际合法组合至少覆盖 start 未决、START 发布中、已接受且运行、fast-exit 暂存、finalizer 已投递和最终完成，合并会增加状态数和非法转移。当前字段逐项职责如下：

| 字段 | 同步方式 | 保留理由 |
|---|---|---|
| `exitPhase` | atomic；复合转移同时持 lifecycle mutex | `Active/Observed/Finalizing/Finalized` 分别表示唯一退出认领、等待 START gate、唯一 timer 投递权、结果完成发布，不能压成单 bool |
| `startPhase` | lifecycle mutex | `Pending/Publishing/Accepted` 保证 START 与 stdout activation 先于 EXIT；失败不再保存可由 `Finalized` 推导的 `Rejected` |
| `terminating` | atomic | stop 可早于 PID/START publication；它同时决定自然退出/restart 语义，不能由 exit code 9 反推 |
| `recovered` | 发布前普通 bool，之后 immutable | 决定无 PM callback 的 polling 与兼容 exit code 0；由 `Application::m_process` 锁完成对象发布，不需要额外 atomic |
| `lastPid` | `m_processMutex` 保护的普通字段 | `m_pid` 在退出认领时失效，但 EXIT payload 与完成后的 `wait` 仍需原 PID |
| `processStartToken` | `m_processMutex` 保护的普通字段 | 与 PID 组成一致活跃快照，防 PID reuse/zombie 误判和误杀；不需要额外 atomic |
| `startError` | lifecycle mutex | native/Docker/cgroup 可分阶段写入，稍后的 start result 与 EXIT error 仍需一致读取 |
| `completionCv` | lifecycle mutex + CV | waiter 等待的是 AppMesh finalization，不再轮询 PM 或 sleep |

`m_pid`、`m_returnValue`、`recovered`、`lastPid` 和 `processStartToken` 位于对象主体而非 lifecycle struct；退出结果在 lifecycle mutex 下先写入，再以 `Observed` 的 release store 发布。主要判断函数保持窄语义：

| 判断/动作 | 唯一语义 |
|---|---|
| `isStartAccepted()` | START publication gate 已打开 |
| `canReportExit()` | 退出仍为 `Active`，polling 路径可以成为第一个 reporter |
| `isFinalized()` | cleanup、Application exit commit 与 waiter publication 已完成 |
| `running()` | 在一致 PID/token 快照上验证同一 OS process 且排除 zombie |
| `wait()` | 只等待 `Finalized`，不负责 OS reaping 或退出状态推进 |
...
- `resolveStart(false)` 不再保存冗余 `Rejected`，只将 exit phase 标记为 `Finalized` 并唤醒等待者。
...
sequenceDiagram
    ...
    P->>P: store code; Active -> Observed
    ...
    P->>T: onExit(code) schedules one-shot
    T->>P: onTimerExit()
...
2. 即使 callback 在 `resolveStart` 之前到达，`onExit(code)` 也只保存 `Observed`；`resolveStart` 完成 START 与 stdout 激活后会用已保存的 exit code 再进入 `onExit(code)`，打开 timer gate。
...
- `onExit(int)`：记录第一次退出；START gate 已打开时认领唯一投递权，并通过 TimerManager 移出 `Process_Manager` upcall。
- `onTimerExit()`：只由 TimerManager 调用，在专用 timer 线程执行资源、状态与 observer 的收口。
...
- `tv == 0`：纯非阻塞完成检查。
- `tv > 0`：使用同一个绝对 deadline 的 CV 谓词循环；只处理虚假唤醒和 `Observed` 重试，没有固定 sleep，也不会延长总 timeout。
...
- recurring retention buffer：native run 保留到 PM callback 完成；Docker/recovered buffer 也会 polling，timer-driven terminate 走同一合成退出入口。
...
3. 在 `m_processMutex` 下取得 PID + OS process start token 的一致快照，验证仍是同一个进程，同时排除 zombie，避免 PID reuse 误杀；自然退出只先失效 PID，避免读者看到“旧 PID + 已清零 token”后退化成 PID-only 判断。
4. 终止 process group；从 PM 移除并按需要回收 child。
5. 合成 exit code 9。
6. 若自然退出 callback 同时到达，lifecycle lock 下的 `Active -> Observed` 只允许一个路径发布结果。

如果 stop 在 start 尚未发布时先到达，`terminating` 会被记住；只要后端最终接受，`AppProcess::start` 会重新进入统一 `terminate()` 并完成 forced exit。`Application` 的 run phase 同时从 `Starting/Running` 进入 `StopRequested`，所以不会把这个退出误判为自然 crash/restart。

合成退出集中在 `AppProcess::terminate()`，不再由 base backend、Application caller 和 timer caller 各补一次。这样 Docker detach、retention timeout 和普通 stop 共享同一收口；native callback 与合成通知由 exit phase 去重。buffer 只有在 `Finalized` 后才从 Application 释放，避免 OS 已退出但 `ExitAdapter` 尚未获得 weak target 时提前销毁对象。
...
`EventDispatcher` 只在短索引临界区内维护订阅和复制 callback；独立的 delivery mutex 串行分配 sequence 并调用不重新进入 EventDispatcher 的 transport delivery callback，期间不持有索引 mutex。这样同一订阅不会并发或乱序投递，失败订阅仍在第二个短索引临界区删除，也不需要 recursive mutex。
...
3. `consumeScheduledStart` 在确认 enabled、到期和时间窗后消费计划；未 `Finalized` 的 process
   仍阻止 continuous restart，interval/cron 只替换仍在运行、已 accepted 且 exit 尚未被认领的 active run，
...
2026-08-19 的增量复核另取约半年前的 `b01d6d609cf3`（2026-02-22）为代码基线，并按本文件的 Mermaid 流程逐边对照。该基线仍由 `execute` 分段调用 unavailable/schedule/refresh/exit handling，以独立 atomics、时间戳和 `ACE_Event` timer handshake 拼接 start/exit；当前实现把它收敛为一个 runtime snapshot、一次 exit claim、一个 scheduler-tick start 入口和一个 CV completion 条件。复核没有再拆 Application 状态或增加单用途状态 helper；新增的只有通用 timer-token ownership 交接和事件 delivery 顺序锁。
...
- recurring process 若 OS 已退出但 PM callback 尚未提交，仍保留为 current 等待 callback；不会把自然退出抢成 replacement 的合成 code 9。
...
AppProcess::m_processMutex
    -> Process_Manager::mutex
        -> AppProcess::Lifecycle::mutex
```

并非每条路径都持有全部锁；这是发生嵌套时的允许方向。关键约束：

- `Process_Manager` upcall 只做 exit phase guard、结果发布与短 lifecycle lock，不调用 `Application`，不拿 `m_processMutex`。
...
- EventDispatcher 的 transport delivery callback 不持有订阅索引锁，但持有独立 delivery 顺序锁；TaskRequest 与 run completion callback 都在各自内部锁外执行外部代码。
- run phase、completion subscription counter 与 callback registry 共用 `runMutex`；订阅、完成和新 run 发布相互原子，不需要第二把 completion lock。
...
TimerManager 的并发边界是：`main.cpp::initializeACE()` 在 reactor/worker 启动前单线程构造 `ACE_Null_Mutex` singleton；同样使用 `ACE_Null_Mutex` 的 EventDispatcher 也在这里显式预初始化。构造完成后，ACE queue adapter 和 heap 允许多个线程并发 register/cancel；保留 token 的 publication/cancel 由短 ownership mutex 串行，token registry 由独立 mutex 保护，锁都不进入 callback。adapter 强制只有一个 dispatch thread，因此 callback 彼此不并行；耗时 callback 会推迟其他 timer。这里的“支持多线程”指多线程调用管理 API，并不表示 timer callback 多线程并行。

静态 lock-order review 未发现已知锁环，也删除了不需要的 recursive mutex。尤其规避了以下危险组合：

- PM mutex → stdout teardown/Application callback → PM mutex；
- exit cleanup 漏过尚未发布 ID 的 terminate/stdout timer，导致 timer 长期持有 process；
- ACE heap ID 复用后，旧 owner 误取消新 exit finalizer/timeout timer；
...
这不是运行时 deadlock 证明。退出 finalization 会在 TimerManager 上执行有界 stdout/event 工作，Docker control/log 调用也受其同步 timeout 约束；Docker CLI cleanup 只启动受管 helper，不在 timer 上等待。本次消除的是无界 wait、固定 sleep polling、脱管线程和明确的锁内外部调用。
...
| `src/daemon/main.cpp` | scheduler 只处理 managed/system-agent 的完整 `execute` 调度与指标逻辑；one-shot 无额外 completion retry 分支。 |
...
| `src/daemon/process/AppProcess.cpp` | 引入 start/exit 双状态机、CV wait、per-process ExitAdapter、PID start token、phase guard 去重、queued finalization、统一 cleanup、START/stdout gate 和 terminate/start 串行。 |
...
| `src/daemon/process/TimerStdoutStrategy.cpp` | 通过 TimerManager 原子 cancel retained token；等待 dispatch mutex；保持原有 stdout event schema。 |
...
保留 `onExit` 与 `onTimerExit` 两层，是因为它们分别表达“记录事实并离开 PM upcall”与“在专用 timer 线程执行真实收口”；继续合并会让 PM callback 在可能持有 PM mutex 的栈上清理资源。
...
| `onTimerAppRemove` | `onTimerRemoval` | 纯 timer callback 统一使用 `onTimer` 前缀 |
...
| clangd `--check --tweaks=` | `TimerHandler.cpp`、`Application.cpp`、`AppTimer.cpp`、`AppProcess.cpp`、`DockerProcess.cpp`、`DockerApiProcess.cpp`、`DailyLimitation.cpp`、`main.cpp` 全部通过，0 errors；禁用与语义诊断无关的 code-action tweak 自检 |
| Clang static analyzer `--analyze` | `TimerHandler.cpp`、`Application.cpp`、`AppProcess.cpp` 通过；未报告本次锁、线程或所有权改动的内存问题，输出仅包含第三方头文件及既有代码 warning |
| C++11 language check | 当前安装的 ACE 在 header 中强制要求 C++14；仅定义 `ACE_HAS_CPP14` 绕过该依赖版本门槛后，以 `-std=c++11 -fsyntax-only -Wthread-safety` 检查 `TimerHandler.cpp`、`AppProcess.cpp`、`Application.cpp`、`main.cpp`，四者均通过，确认本次新增代码未使用 C++14/17 语法；这不表示当前 ACE 组合支持完整 C++11 构建 |
...
### 18.1 2026-08-19 增量复核记录

| 检查 | 结果 |
|---|---|
| code-simplifier 复核 | 保留现有 start/exit/run/schedule 状态机；未给 Application 或 AppProcess 增加 phase、flag 或单用途状态 helper；ABA 修复只在 TimerHandler/TimerManager 增加逻辑 token→event identity registry |
| clangd `--check --tweaks=` | `TimerHandler.cpp`、`Application.cpp`、`AppProcess.cpp`、`TimerStdoutStrategy.cpp`、`EventDispatcher.cpp`、`HttpRequest.cpp`、`main.cpp` 通过，0 errors |
| `cmake --build build -j2` | 通过，`[100%] Built target appmesh`；仅见上述既有第三方/linker warning |
| `otool -tvV libACE.dylib` | 确认 `ACE_Event_Handler_var` copy 调用 `add_reference()`、析构调用 `remove_reference()`；registry/cancel 的 callback pin 与设计一致 |
| `git diff --check` | 通过 |

该增量复核没有运行项目 C++ binary、C++ UT、Rust、Python/integration workload 或 eval；只执行 LSP、编译/链接和 `otool` 静态检查。
