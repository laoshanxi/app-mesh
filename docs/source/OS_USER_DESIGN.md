...
- A **Principal** is the immutable App Mesh identity derived from a validated access token. Roles and
...
`authorization.yaml`. It is not stored in the authentication service, an upstream identity provider, or a legacy Engine user record.

```text
access token -> (issuer, subject) -> Principal ID -> execution_user -> native child UID/GID
```
...
bindings to the issuer and subject; display names are not authorization keys.
...
Do not put passwords, OAuth client secrets, or access tokens in this file.

## Resolution and Enforcement

For REST-created applications, the effective native user is resolved as follows:

1. Validate the bearer and resolve its immutable Principal.
