...
- `dispatch()` uses a separate delivery mutex to assign sequence numbers and invoke transport delivery callbacks serially; this preserves per-subscription order without holding the subscription-index mutex in a callback
- the delivery mutex is intentionally global to avoid per-subscription queue/lock state; transport callbacks must remain bounded and must not re-enter `EventDispatcher`, because one slow delivery delays later server-side emissions
