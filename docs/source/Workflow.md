...
|------------|--------------------|-------------------------|
| Manual (`appm workflow run`) | caller's Engine-validated Dex bearer | caller's immutable Principal ID |
...
The Engine, not the workflow process, validates Dex issuer, signature, audience, and token
times and maps the bearer to an immutable Principal ID. The workflow engine removes the
bearer from request data after validation and never writes it to run records or logs.
...
allowed by the current owner's RBAC; no Dex service Principal or client secret is involved.
