...
an OAuth access token in the `Authorization: Bearer` header.
...
identity-provider administration. Those operations belong to the authentication service or its
...
login or cookie persistence; `OAuthClient` implements the OAuth flows
...
- [MCP Server](https://github.com/laoshanxi/app-mesh/tree/main/src/sdk/mcp_server) — OAuth-protected Streamable HTTP resource server
