...
`APP_MESH_PROCESS_KEY`. A delegated daemon call may use the Engine-validated human
...
- The original catalog (`llm_agent/tools.py`) was built by listing Apps under a manual
  run's bearer and filtering to those with a `metadata.tool` schema. Tool
...
| L3 | What may a delegated tool do? | App Mesh RBAC for the effective Principal |

- **Manual workflow run:** the Engine validates the caller's bearer and maps it to an
...
- **Token expiry:** delegated calls fail closed when their bearer expires. The agent
...
| Identity | Task RPC via `APP_MESH_PROCESS_KEY`; only manual runs may delegate the Engine-validated human bearer; internal workflow capabilities never leave Engine transport; session ownership uses immutable Principal IDs |
