...
- **Identity** — manual runs use the caller's Engine-validated Dex bearer; automatic and recovered execution uses a short-lived local Engine capability bound to the current owner and managed Workflow process. Owners and human actors are immutable Principal IDs; a new automatic Run uses the explicit `internal:workflow-trigger` audit marker, while recovery preserves the original actor/source. Registering an automatic trigger requires `workflow-admin` (ADR 0006 and ADR 0009).
