...
The executor creates a forwarding TCP client (`ForwardTo`) for remote RunAppAsync,
Subscribe, RunTask, and cleanup operations.
...
`node_label` remains available for manually triggered workflows in a cluster where every
node validates tokens against the same canonical issuer. Local steps use the renewable,
node-local run capability. Remote label probes, step calls, and cancellation carry only the
human caller's transient access token through `X-Target-Host`; that token is verified and
authorized again by the destination Engine. The local run capability is never forwarded.
The caller token must still be unexpired when each remote request is made; the workflow
process does not receive or persist the caller's refresh token.

Automatic and recovered runs do not retain a human bearer, so a remote-only label fails
closed with an explicit credential error. They can still use `node_label` when it matches the
local node. Supporting unattended remote jobs later requires a dedicated least-privilege
service identity; the workflow engine must not persist user refresh tokens or passwords.
...
- **Cross-node:** Manual runs may use `node_label` when all nodes trust one issuer; forwarded requests use the caller bearer and are re-authorized by the destination.
