    ...
    if [[ "$OSTYPE" == "linux"* ]]; then
        ...
        if [[ -n "$LIB_READLINE_VER" ]]; then
            ...
            for bin in appc appsvc; do
                echo "Patching readline for: ${bin}"
...
build_packages() {
    ...
    if grep -q '\${[^}]*}' "${CMAKE_BINARY_DIR}/nfpm_config.yaml"; then
        echo "Error: Some variables were not substituted in nfpm.yaml."
        exit 1
    fi
    ...
    if [[ "$OSTYPE" == "linux"* ]]; then
        ...
        echo "Building for Linux (GLIBC: $GLIBC_VERSION, GCC: $GCC_VERSION, ARCH: $ARCH)"
        ...
        for pkg in appmesh*.{rpm,deb}; do
            ...
            mv "$pkg" "${PACKAGE_FILE_NAME}" && echo "Package built: ${PACKAGE_FILE_NAME}"
        done
        ...
        echo "Building for macOS (Version: $MACOS_VERSION, Clang: $CLANG_VERSION, ARCH: $GOARCH)"

        tar czvf ${PACKAGE_FILE_NAME} -C "${PACKAGE_HOME}" . && echo "Package built: ${PACKAGE_FILE_NAME}"
    else
