...
install_systemd_service() {
    ...
    ln -sf "$service_template" "$SYSTEMD_FILE"
...
install_initd_service() {
    ...
    ln -sf "$initd_template" "$INITD_SOFTLINK"
...
main() {
    ...
    if [ -d "$BASH_COMPLETION_DIR" ]; then
        ln -sf "${PROG_HOME}/script/bash_completion.sh" "$BASH_COMPLETION_SOFTLINK"
    ...
    # Create appc symlink
    ln -sf "${PROG_HOME}/script/appc.sh" "$APPC_SOFTLINK"
