...
readonly AUTH_APP_NAMES=(auth-dex)

DEX_ACCESS_URL_EXPLICIT=0
DEX_ISSUER_EXPLICIT=0
DEX_TLS_VERIFY_EXPLICIT=0
DEX_CA_PATH_EXPLICIT=0
PREVIOUS_AUTH_MODE=""
[ "${APPMESH_DEX_ACCESS_URL+x}" = x ] && DEX_ACCESS_URL_EXPLICIT=1
[ "${APPMESH_DEX_ISSUER+x}" = x ] && DEX_ISSUER_EXPLICIT=1
[ "${APPMESH_DEX_TLS_VERIFY+x}" = x ] && DEX_TLS_VERIFY_EXPLICIT=1
[ "${APPMESH_DEX_CA_PATH+x}" = x ] && DEX_CA_PATH_EXPLICIT=1
...
Authentication options (Linux/macOS):
  --auth-mode builtin|external  Select the bundled or external Dex issuer (default: builtin)
  --oidc-issuer URL             Canonical external Dex issuer (required for external mode)
...
APPMESH_DEX_ISSUER, APPMESH_DEX_ACCESS_URL, APPMESH_DEX_TLS_VERIFY, and
APPMESH_DEX_CA_PATH. This interface never accepts user or client passwords.
EOF
}
...
parse_arguments() {
    while [ "$#" -gt 0 ]; do
        ...
        --oidc-issuer)
            require_option_value "$1" "${2:-}"
            export APPMESH_DEX_ISSUER="$2"
            DEX_ISSUER_EXPLICIT=1
            shift 2
            ;;
        --oidc-issuer=*) export APPMESH_DEX_ISSUER="${1#*=}"; DEX_ISSUER_EXPLICIT=1; shift ;;
        --oidc-access-url)
            require_option_value "$1" "${2:-}"
            export APPMESH_DEX_ACCESS_URL="$2"
            DEX_ACCESS_URL_EXPLICIT=1
            shift 2
            ;;
        --oidc-access-url=*) export APPMESH_DEX_ACCESS_URL="${1#*=}"; DEX_ACCESS_URL_EXPLICIT=1; shift ;;
        --oidc-tls-verify)
            require_option_value "$1" "${2:-}"
            export APPMESH_DEX_TLS_VERIFY="$2"
            DEX_TLS_VERIFY_EXPLICIT=1
            shift 2
            ;;
        --oidc-tls-verify=*) export APPMESH_DEX_TLS_VERIFY="${1#*=}"; DEX_TLS_VERIFY_EXPLICIT=1; shift ;;
        --oidc-ca-path)
            require_option_value "$1" "${2:-}"
            export APPMESH_DEX_CA_PATH="$2"
            DEX_CA_PATH_EXPLICIT=1
            shift 2
            ;;
        --oidc-ca-path=*) export APPMESH_DEX_CA_PATH="${1#*=}"; DEX_CA_PATH_EXPLICIT=1; shift ;;
        --clear-oidc-ca)
            export APPMESH_DEX_CA_PATH=""
            DEX_CA_PATH_EXPLICIT=1
            shift
            ;;
        -h | --help)
...
normalize_tls_verify() {
    ...
    *) die "APPMESH_DEX_TLS_VERIFY must be true or false" ;;
    esac
}
...
configure_authentication() {
    ...
    *) die "APPMESH_AUTH_MODE must be builtin or external" ;;
    ...
    issuer=$(read_env_entry APPMESH_DEX_ISSUER 2>/dev/null || true)
    access_url=$(read_env_entry APPMESH_DEX_ACCESS_URL 2>/dev/null || true)
    tls_verify=$(read_env_entry APPMESH_DEX_TLS_VERIFY 2>/dev/null || true)
    ca_path=$(read_env_entry APPMESH_DEX_CA_PATH 2>/dev/null || true)

    if [ "$mode" = "external" ]; then
        if [ "$PREVIOUS_AUTH_MODE" != "external" ] && [ "$DEX_ISSUER_EXPLICIT" -ne 1 ]; then
            die "Changing to external authentication requires an explicitly supplied --oidc-issuer or APPMESH_DEX_ISSUER"
        fi
        ...
        if [ "$PREVIOUS_AUTH_MODE" != "external" ]; then
            [ "$DEX_ACCESS_URL_EXPLICIT" -eq 1 ] || access_url=""
            [ "$DEX_TLS_VERIFY_EXPLICIT" -eq 1 ] || tls_verify=""
            [ "$DEX_CA_PATH_EXPLICIT" -eq 1 ] || ca_path=""
        fi
        [ -n "$issuer" ] || die "External authentication requires --oidc-issuer or APPMESH_DEX_ISSUER"
        access_url=${access_url:-$issuer}
        tls_verify=$(normalize_tls_verify "${tls_verify:-true}")
        validate_oidc_url APPMESH_DEX_ISSUER "$issuer"
        validate_oidc_url APPMESH_DEX_ACCESS_URL "$access_url"
        if [ -n "$ca_path" ]; then
            [[ "$ca_path" = /* ]] || die "APPMESH_DEX_CA_PATH must be an absolute path"
            [ -e "$ca_path" ] || die "APPMESH_DEX_CA_PATH does not exist: $ca_path"
        fi
        write_env_entry "$ENV_FILE" APPMESH_DEX_ISSUER "$issuer"
        write_env_entry "$ENV_FILE" APPMESH_DEX_ACCESS_URL "$access_url"
        write_env_entry "$ENV_FILE" APPMESH_DEX_TLS_VERIFY "$tls_verify"
        if [ -n "$ca_path" ]; then
            write_env_entry "$ENV_FILE" APPMESH_DEX_CA_PATH "$ca_path"
        elif [ "$DEX_CA_PATH_EXPLICIT" -eq 1 ] || [ "$PREVIOUS_AUTH_MODE" != "external" ]; then
            remove_env_entry "$ENV_FILE" APPMESH_DEX_CA_PATH
        fi
        set_auth_app_status 0
        info "Authentication mode set to external; bundled Dex System Apps are disabled"
    else
        ...
        # retained: it may be the public URL of the newly local Dex behind an
        # ingress.
        if [ "$PREVIOUS_AUTH_MODE" = "external" ]; then
            if [ "$DEX_ACCESS_URL_EXPLICIT" -eq 0 ]; then
                remove_env_entry "$ENV_FILE" APPMESH_DEX_ACCESS_URL
                access_url=""
            fi
            if [ "$DEX_TLS_VERIFY_EXPLICIT" -eq 0 ]; then
                remove_env_entry "$ENV_FILE" APPMESH_DEX_TLS_VERIFY
                tls_verify=""
            fi
            if [ "$DEX_CA_PATH_EXPLICIT" -eq 0 ]; then
                remove_env_entry "$ENV_FILE" APPMESH_DEX_CA_PATH
                ca_path=""
            fi
        elif [ "$DEX_CA_PATH_EXPLICIT" -eq 1 ] && [ -z "$ca_path" ]; then
            remove_env_entry "$ENV_FILE" APPMESH_DEX_CA_PATH
        fi
        [ -z "$issuer" ] || validate_oidc_url APPMESH_DEX_ISSUER "$issuer"
        [ -z "$access_url" ] || validate_oidc_url APPMESH_DEX_ACCESS_URL "$access_url"
        [ -z "$tls_verify" ] || normalize_tls_verify "$tls_verify" >/dev/null
        set_auth_app_status 1
        info "Authentication mode set to builtin; bundled Dex System Apps are enabled"
        # Without the launcher installed, the Dex App self-bootstraps instead.
...
prepare_workflow_app() {
    ...
    # registration because Workflow is no longer a Dex service client.
    ...
    if [ -f "$WORKFLOW_DISABLED_APP" ]; then
        ...
        # local Dex. The current template is mode-independent.
...
setup_permissions() {
    ...
    chown -R "root:${root_group}" "${PROG_HOME}"
    find "${PROG_HOME}" -path "${PROG_HOME}/work" -prune -o -type d -exec chmod go-w {} \;
    find "${PROG_HOME}" -path "${PROG_HOME}/work" -prune -o -type f -exec chmod go-w {} \;
...
setup_ssl_certificates() {
    ...
    local root_group=""
    root_group=$(id -gn 0)
    find "$ssl_dir" -type f -name "*.pem" -exec chown "root:${root_group}" {} \;
    find "$ssl_dir" -type f -name "*.pem" ! -name "*-key.pem" -exec chmod 644 {} \;
    find "$ssl_dir" -type f -name "*-key.pem" -exec chmod 600 {} \;

    if [ -n "${APPMESH_DAEMON_EXEC_USER:-}" ]; then
        local daemon_group="${APPMESH_DAEMON_EXEC_USER_GROUP:-}"
        daemon_group=${daemon_group:-$(id -gn "${APPMESH_DAEMON_EXEC_USER}")}
        # The daemon needs its server and loopback-client keys. The CA signing
        # key remains root-only and is never needed at runtime.
        for private_key in server-key.pem client-key.pem; do
            if [ -f "${ssl_dir}/${private_key}" ]; then
                chown "root:${daemon_group}" "${ssl_dir}/${private_key}"
                chmod 640 "${ssl_dir}/${private_key}"
            fi
        done
    fi
}
...
print_startup_instructions() {
    ...
    *)
    ...
    if [ "$(read_env_entry APPMESH_AUTH_MODE 2>/dev/null || true)" = "builtin" ]; then
        info "  First login: after the service starts, read the password field from"
        info "    ${PROG_HOME}/work/auth/secrets/dex-initial-admin-credentials"
        info "  The read-only viewer signs in as guest@appmesh.local with the password in"
        info "    ${PROG_HOME}/work/auth/secrets/dex-initial-guest-credentials"
        info "  Run sudo appm logon --enroll-first-admin locally and sign in as admin@appmesh.local."
        info "  After enrollment, remove recoverable plaintext with:"
        info "    sudo ${PROG_HOME}/script/appmesh-auth.sh forget-initial-password"
    fi
}
